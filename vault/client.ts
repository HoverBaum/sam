...
function parseJson(stdout: string, commandName: string): unknown {
  ...
  throw new Error(
    `${commandName} expected JSON output but got non-JSON content.`,
  )
}
...
function normalizeOutlineLevel(
...
export function parseOutlineStdout(stdout: string): OutlineNode[] {
  ...
  if (
    lower === 'no headings found' ||
    lower === 'no headings found.' ||
    lower.includes('no headings found')
  ) {
    return []
  }
...
async function runObsidian(
  ...
  const args =
    includeVault && config.vault ? [`vault=${config.vault}`, ...tokens] : tokens
  ...
  if (result.code !== 0) {
    ...
    throw new Error(
      `obsidian ${args.join(' ')} failed (${result.code})${stderr ? `: ${stderr}` : ''}`,
    )
  }
...
export class VaultClient {
  ...
  async listVaultNames(): Promise<string[]> {
    const stdout = await runObsidian(this.config, ['vaults'], {
      includeVault: false,
    })
  ...
  async outline(path: string): Promise<OutlineNode[]> {
    const stdout = await runObsidian(this.config, [
      'outline',
      `path=${path}`,
      'format=json',
    ])
  ...
  async backlinks(path: string): Promise<BacklinkEntry[]> {
    ...
    return rows
      .map((row): BacklinkEntry | undefined => {
        ...
        const rec = row as Record<string, unknown>
        const sourcePath = readString(rec, [
          'sourcePath',
          'source',
          'path',
          'file',
