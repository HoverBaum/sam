...
interface UseShellWorkspaceResult {
  terminalRows: number
...
function draftFromConfig(
  ...
  return {
    ...
    embeddingProvider:
      file.embeddingProvider ?? context.config.embeddingProvider ?? '',
    embeddingModel: file.embeddingModel ?? context.config.embeddingModel ?? '',
    embeddingBaseUrl:
      file.embeddingBaseUrl ?? context.config.embeddingBaseUrl ?? '',
  }
}
...
export function useShellWorkspace(
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const changed =
          summary.newPaths.length +
          summary.modifiedPaths.length +
  ...
  const openSettingsMenu = async () => {
    ...
    navigate('/config')
    pushMessage(
      '🎛️  Settings opened. Route is now /config. ↑/↓ select · Enter edit · Esc shell.',
    )
  }
  ...
  const beginEditingField = (field: SettingsField) => {
    ...
    pushMessage(
      `Editing ${FIELD_LABELS[field]} at ${settingsFieldPath(field)}. Tab suggestions · Enter apply · Esc settings.`,
    )
  }
  ...
  const saveSettings = async () => {
    ...
    const merged: SamConfigFile = {
      ...
      embeddingProvider:
        (settingsDraft.embeddingProvider as SamConfigFile['embeddingProvider']) ||
        undefined,
    ...
    if (editingField) {
      ...
      if (key.tab && autoSuggestions.length > 0) {
        ...
        setAutocompleteIndex(
          (idx: number) => (idx + 1) % autoSuggestions.length,
        )
        return
      }
    ...
    if (isSettingsMenu) {
      ...
      if (char?.toLowerCase() === 's') {
        try {
          ...
          pushMessage(
            `Failed to save settings: ${String((error as Error).message ?? error)}`,
          )
        }
        return
      }
      ...
      if (command.kind === 'navigate') {
        if (command.path === '/config') {
          try {
            ...
            pushMessage(
              `Failed to open settings: ${String((error as Error).message ?? error)}`,
            )
          }
          return
        }
